## Iteration 1

>start/end date: 25/02/26 - 18/03/26

- [x] manage documentation in a github repository

    - [x] created github repository - 25/02/26
<br>

- [x] meetings (4 total, 1 for each week)

    - [x] meeting minutes #1 - 25/02/26 (15 mins)
    - [x] meeting minutes #2 - 04/03/26 (15 mins)
    - [x] meeting minutes #3 - 10/03/26 (15 mins)
    - [x] meeting minutes #4 - 11/03/26 (15 mins)
<br>

- [x] Stakeholder Register

    - [x] created stakeholder register - 25/02/26 (deadline: 29/02/2026 *4 days* - ref: meeting #1)
<br>

- [x] 3 personas (1 each) - 04/03/26 (deadline: 10/03/26 *6 days* - ref: meeting #2)

    - [x] persona #1 - 12/03/26
    - [x] persona #2 - 12/03/26
    - [x] persona #3 - 12/03/26
<br>

- [x] Review Personas as a group - 10/03/26 (deadline: 17/03/26 *7 days* - ref: meeting #3)

    - [x] persona reviews - 17/03/26
<br>

- [x] README file - 10/03/26 (deadline: 17/03/26 *7 days* - ref: meeting #3)

    - [x] added persona screenshots - 11/03/26
    - [x] added progress report - 17/03/26
<br>

- [x] progress report (150 words min.) - 10/03/26 (deadline: 17/03/26 *7 days* - ref: meeting #3)
    *individual progress report from each group member*

    - [x] progress report submission - 17/03/26
<br>

- [x] iteration 1 completed - due date: 18/03/26

---

## Iteration 2

>start/end date: 18/03/26 - 29/04/26

- [x] weekly meeting reports (4 reports min.)
    *meeting agenda, date, actions, attendees, info about next meeting*

    - [x] meeting minutes #1 - 18/03/26
    - [x] meeting minutes #2 - 25/03/26
    - [x] meeting minutes #3 - 01/04/26
    - [x] meeting minutes #4 - 22/04/26
<br>

- [x] detailed Gantt Chart - 01/04/26 (deadline: 28/04/26 - ref: meeting #3 & meeting #4)
    *task requirements, start date, end date, completed by*

    - [x] gantt chart created - 01/04/26
    - [x] gantt chart iteration 1 - 01/04/26 - 24/04/26
    - [x] gantt chart iteration 2 - 22/04/26 - 28/04/26
<br>

- [x] refer to stakeholder reg, assign tasks in Gantt Chart
    *record info into GitHub boards*

    - [x] created kanban - 25/03/26
      - update kanban - 22/04/26
<br>

- [x] complete Resource Requirement List - 22/04/26 (deadline: 28/04/26 - ref: meeting #4)
    *explain need for resources, resource name, description, cost, task code*

    - [x] push resource template to repo - 22/04/26

- [x] as group, test and review new changes/README.md - 28/04/26
<br>

- [x] individual report - 28/04/26 (29/04/26 at latest, if needed)

    - [x] progress report submission - 28/04/26

- [x] iteration 2 completed - 29/04/26

---

## Iteration 3

>start/end date: 29/04/26 - 27/05/26

- [ ] weekly meeting reports (4 reports min.)
    *meeting agenda, date, actions, attendees, info about next meeting*

    - [x] meeting minutes #1 - 29/04/26
    - [x] meeting minutes #2 - 06/05/26
    - [x] meeting minutes #3 - 13/05/26
    - [x] meeting minutes #4 - 20/05/26
    *update kanban and gantt chart - note schedule change: due 27/05/26*
<br> 

- [x] risk register (template available) - 13/05/26 (deadline: 19/05/26 - ref: meeting #3)
    *identify and manage actual risks during project progress*
    - [x] include: description of risk, likelihood, impact, severity, clear resolve plan (mitigation response) and owner (who raised risk)
<br>

- [x] push website files to github - 06/05/26 (deadline: 06/05/26)
    *push and merge to repo - make an edit*
    - [x] member 1 & 3 - html/css/js/image and index.html - abby - 06/05/26
    - [x] member 2 - orders.html - sara - 06/05/26
    - [x] member 4 - products.html - aiden - 06/05/26
<br>

- [x] check integration and publish - 13/05/26 published (ref: meeting #2)
    *each page has unique code, each group member needs to merge and test complete format*
    - [x] issue - move from /root to /docs
<br>

- [x] progress report *CHANGED FROM 20/05/26 to 27/05/26
- [x] update readme with iteration 3
<br>

- [ ] iteration 3 complete - *CHANGED FROM 20/05/26 to 27/05/26
- [ ] prepare documents for handover - 27/05/26 (due end of week 13)